...
Future<ApiResponse> getSharedtopics() async {
  ApiResponse apiResponse = ApiResponse();
  try {
    String token = await getToken();
    final response = await http.get(Uri.parse(sharedtopicsURL), headers: {
    ...
    switch (response.statusCode) {
      case 200:
        apiResponse.data = jsonDecode(response.body)['sharedtopics']
            .map((p) => SharedTopic.fromJson(p))
            .toList();
